...
void material::serialize(std::ostream&) const
{
	/* Empty on purpose. */
}

color_material::color_material(const vec4f& diffuse_color)
: diffuse_color(diffuse_color)
{
	/* Empty on purpose. */
}

void color_material::bind() const BOOST_NOEXCEPT_OR_NOTHROW
{
...
void color_material::serialize(std::ostream& os) const
{
	os << "diffuse_color: " << diffuse_color << '\n';
}

color_material::color_material(std::istream& is)
{
	is.ignore(strlen("diffuse_color: "));
	is >> diffuse_color;
}
...
void diffuse_material::serialize(std::ostream& os) const
{
	os << "diffuse: " << std::get<0>(*m_diffuse) << '\n';
	os << "diffuse_factor: " << diffuse_factor << '\n';
}

diffuse_material::diffuse_material(std::istream& is)
{
	boost::filesystem::path diffuse_filename;
	is.ignore(strlen("diffuse: "));
	is >> diffuse_filename;
	is.ignore(strlen("diffuse_factor: "));
	is >> diffuse_factor;
	safe_obtain_texture(diffuse_filename, m_diffuse);
}
...
material_cd::material_cd(std::istream& is)
: color_material(is)
{
	boost::filesystem::path diffuse_filename;
	is.ignore(strlen("diffuse: "));
...
void material_cd::serialize(std::ostream& os) const
{
	color_material::serialize(os);
	os << "diffuse: " << std::get<0>(*m_diffuse) << '\n';
}
...
void specular_diffuse_material::serialize(std::ostream& os) const
{
	diffuse_material::serialize(os);
	os << "specular: " << std::get<0>(*m_specular) << '\n';
	os << "shininess: " << shininess << '\n';
}

specular_diffuse_material::specular_diffuse_material(std::istream& is)
: diffuse_material(is)
{
	boost::filesystem::path specular_filename;
	is.ignore(strlen("specular: "));
	is >> specular_filename;
	is.ignore(strlen("shininess: "));
	is >> shininess;
	safe_obtain_texture(specular_filename, m_specular);
}
