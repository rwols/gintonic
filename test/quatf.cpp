...
{
    quatf a = quatf::axis_angle(vec3f(0.0f, 1.0f, 0.0f),
                                static_cast<float>(M_PI) / 2.0f);
    quatf b = quatf::axis_angle(vec3f(0.0f, 0.0f, -1.0f),
                                static_cast<float>(M_PI) / 2.0f);
    ...
    BOOST_CHECK_CLOSE(
        dir.x, -1.0f,
        0.001f); // note that quaternion multiplication is not commutative ...
    BOOST_CHECK_CLOSE(dir.y, 0.0f, 0.001f);
    BOOST_CHECK_CLOSE(dir.z, 0.0f, 0.001f);

    // start with the vector [0,0,-1], rotate it CCW 90 degrees around [0,1,0],
    // which results in the vector [-1,0,0], then rotate it CCW 90 degrees
    // around [0,0,-1]. This is the same as rotating CW 90 degrees around
    // [0,0,1], which results in the vector [0,1,0].
    dir = (b * a).direction();
...
{
    auto a = quatf::axis_angle(vec3f(0.0f, 1.0f, 0.0f),
                               static_cast<float>(M_PI) / 2.0f);
    ...
    for (float lambda = 0.0f; lambda <= 1.0f; lambda += 0.001f)
    {
        ...
        auto diff = q - prev_q;
        GINTONIC_CHECK_VECTOR4_SMALL(diff, 0.001f);
        prev_q = q;
    }
}
...
BOOST_AUTO_TEST_CASE(slerp_test)
{
    ...
    for (float lambda = 0.0f; lambda <= 1.0f; lambda += 0.001f)
    {
        ...
        GINTONIC_CHECK_VECTOR4_SMALL((q - prev_q), 0.001f);
        prev_q = q;
    }
}
