...
class Entity : public Object<Entity, std::string>
{
  ...
  public:
    /// \brief Calls update on all of its components.
    void update();

    /// \brief Calls lateUpdate on all of its components.
    void lateUpdate();
    ...
    /**
     * @brief Creates a new Entity::SharedPtr with the given constructor
     * arguments. Note that this is the *only* way to create new a new Entity.
     * It's impossible to create an Entity on the stack *by design*, and it is
     * also impossible to create a new Entity with std::make_shared *by design*.
  ...
  public:
     ...
     */
    inline std::weak_ptr<Entity> parent() noexcept { return mParent; }
