...
#define GINTONIC_DEFINE_ALIGNED_OPERATOR_NEW_DELETE(alignment)                 \
    ...
    inline static void operator delete(void* ptr) { _mm_free(ptr); }           \
    inline static void operator delete[](void* ptr) { _mm_free(ptr); }         \
...
typename std::enable_if<!std::numeric_limits<T>::is_integer,
                        bool>::type inline almost_equal(const T x, const T y,
                                                        int units_in_last_place)
{
 ...
 */
inline float dot(const __m128& lhs, const __m128& rhs) noexcept
{
 ...
 */
inline float deg2rad(const float t) { return float(0.0174532925) * t; }
 ...
 */
inline float rad2deg(const float t) { return float(57.2957795) * t; }
