...
#include <cstddef>
#include <gintonic/Foundation/simd.hpp>
#include <memory>
...
template <class T, std::size_t Alignment = 16> class allocator
{
  public:
     ...
     */
    inline T* address(T& t) const { return std::addressof(t); }
     ...
     * @return A pointer to the object.
     */
    inline const T* address(const T& t) const { return std::addressof(t); }
     ...
     */
    inline void construct(T* const p, const T& t) const { new (p) T(t); }

    /**
     * @brief Explicitly call the destructor of an object.
     * @param p A constant pointer to destroy.
     */
    inline void destroy(T* const p) const { p->~T(); }

    /// Always returns true.
    inline bool operator==(const allocator& other) const { return true; }
    ...
    /// Copy constructor from another template type U, but with the same
    /// alignment.
