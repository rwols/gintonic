...
{
  public:
    class EntityNotContainedInOctreeBoundingBox : public std::out_of_range
    {
      public:
        ~EntityNotContainedInOctreeBoundingBox() noexcept override = default;
        inline Entity& getEntity() noexcept { return mEntity; }
        inline const Entity& getEntity() const noexcept { return mEntity; }
        inline Octree* getOctreeNode() noexcept { return mOctreeNode; }
        inline const Octree* getOctreeNode() const noexcept
        {
            return mOctreeNode;
        }
      ...
      private:
        ...
        EntityNotContainedInOctreeBoundingBox(A&& octree, B&& entity)
            : std::out_of_range("EntityNotContainedInOctreeBoundingBox"),
              mOctreeNode(std::forward<A>(octree)),
              mEntity(std::forward<B>(entity))
        {
            /* Empty on purpose. */
        }
        Octree* mOctreeNode;
  ...
  private:
    struct EntityHolder
    {
        ...
        template <class A> EntityHolder(A&& a) : entity(std::forward<A>(a))
        {
            /* Empty on purpose. */
        }
    ...
    Octree* mChild[8] = {nullptr, nullptr, nullptr, nullptr,
  ...
  public:
    ...
    Octree(const box3f& b, ForwardIter first, ForwardIter last);
     ...
     */
    template <class ForwardIter>
    Octree(const vec3f& minCorner, const vec3f& maxCorner, ForwardIter first,
           ForwardIter last);
     ...
     */
    inline const box3f& bounds() const noexcept { return mBounds; }
     ...
     */
    inline bool isRoot() const noexcept { return !mParent; }
     ...
     */
    inline bool hasNoEntities() const noexcept { return mEntities.empty(); }
     ...
     */
    template <class OutputIter> void getEntities(OutputIter iter);
     ...
     * @param iter An output iterator.
     */
    template <class OutputIter> void getEntities(OutputIter iter) const;
     ...
     */
    template <class Func> void foreach (Func f);
    ...
    template <class Func> void forEachNode(Func f);

    template <class Func> void forEachNode(Func f) const;
  ...
  private:
    ...
    Octree(const float subdivisionThreshold, Octree* parent, const vec3f& min,
           const vec3f& max);
    ...
    box3f getBounds(const Entity& entity) const noexcept;

    template <class Archive> void save(Archive& archive, const unsigned version)
    {
        if (mParent)
        {
            getRoot()->save(archive, version);
        }
        else
        {
            archive& mBounds;
            archive& subdivisionThreshold;
            saveRecursive(archive, version);
        }
    }

    template <class Archive>
    void saveRecursive(Archive& archive, const unsigned version)
    {
        archive& mEntities;
        if (mAllocationPlace != nullptr)
        {
            static const char sIsNotLeaf = 0;
            archive& sIsNotLeaf;
            for (const auto* lChild : mChild)
            {
                lChild->saveRecursive(archive, version);
            }
        }
        else
        {
            static const char sIsLeaf = 1;
            archive& sIsLeaf;
        }
    }

    template <class Archive> void load(Archive& archive, const unsigned version)
    {
        if (mParent)
        {
            getRoot()->load(archive, version);
        }
        else
        {
            archive& mBounds;
            archive& subdivisionThreshold;
            loadRecursive(archive, version);
        }
    }

    template <class Archive>
    void loadRecursive(Archive& archive, const unsigned version)
    {
        archive& mEntities;
        char lIsLeaf;
        archive& lIsLeaf;
        if (lIsLeaf)
        {
            mChild[0] = mChild[1] = mChild[2] = mChild[3] = mChild[4] =
                mChild[5] = mChild[6] = mChild[7] = nullptr;
        }
        else
        {
            subdivide();
            for (auto* childNode : mChild)
            {
                childNode->loadRecursive(archive, version);
            }
        }
    }
...
template <class ForwardIter>
Octree::Octree(const box3f& bounds, ForwardIter first, ForwardIter last)
    : mBounds(bounds)
{
    mChild[0] = mChild[1] = mChild[2] = mChild[3] = mChild[4] = mChild[5] =
...
template <class OutputIter> void Octree::getEntities(OutputIter iter)
{
    for (auto& holder : mEntities)
    {
        *iter = &holder.entity;
        ++iter;
    }
    for (auto* childNode : mChild)
    {
        if (childNode) childNode->getEntities(iter);
    }
}

template <class OutputIter> void Octree::getEntities(OutputIter iter) const
{
    for (const auto& holder : mEntities)
    {
        *iter = &holder.entity;
        ++iter;
    }
    for (const auto* childNode : mChild)
    {
        if (childNode) childNode->getEntities(iter);
    }
}
...
void Octree::getEntities(OutputIter iter, FilterFunc filter)
{
    ...
    for (auto* childNode : mChild)
    {
        if (childNode) childNode->getEntities(iter, filter);
    }
}
...
void Octree::query(const box3f& volume, OutputIter iter)
{
    for (auto& holder : mEntities)
    {
        if (intersects(volume, getBounds(holder.entity)))
        {
            *iter = &holder.entity;
            ++iter;
        }
    ...
    for (auto* childNode : mChild)
    {
        ...
        // +-----+-----+
        //
        if (childNode->mBounds.contains(volume))
        {
            childNode->query(volume, iter);
            break;
        }
        ...
        //   +-----+-----+
        //
        else if (volume.contains(childNode->mBounds))
        {
            childNode->getEntities(iter);
        }
        ...
        //   +-----+-----+
        //
        else if (intersects(volume, childNode->mBounds))
        {
            childNode->query(volume, iter);
        }
...
void Octree::query(const box3f& volume, OutputIter iter, FilterFunc filter)
{
    ...
    for (auto* childNode : mChild)
    {
        ...
        if (childNode->mBounds.contains(volume))
        {
            childNode->query(volume, iter, filter);
            break;
        }
        else if (volume.contains(childNode->mBounds))
        {
            childNode->getEntities(iter, filter);
            continue;
        }
        else if (intersects(volume, childNode->mBounds))
        {
...
template <class Func> void Octree::foreach (Func f)
{
...
template <class Func> void Octree::foreach (Func f) const
{
    for (const auto& o : mEntities)
    {
        f(o.entity);
    }
    if (mAllocationPlace == nullptr) return;
...
template <class Func> void Octree::forEachNode(Func f)
{
    f(this);
    if (mAllocationPlace == nullptr) return;
    for (auto* childNode : mChild)
        if (childNode) childNode->forEachNode(f);
}

template <class Func> void Octree::forEachNode(Func f) const
{
    f(this);
    if (mAllocationPlace == nullptr) return;
    for (auto* childNode : mChild)
