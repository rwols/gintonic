...
#include <gintonic/Foundation/WriteLock.hpp>
...
class Renderer
{
  public:
    ...
    /**
     * @brief Gets thrown when not a single display is found during
     * initialization.
     */
    ...
    class NoContextAvailableException : public std::exception
    {
      public:
         ...
         */
        inline NoContextAvailableException(const int major,
     ...
     * then this function will add a default Camera having a perspective
     * projection.
     ...
     * @param preferredHeight The preferred height of the window. Note that if
     * the
     * fullscreen parameter is set to true, then the current resolution is
     * taken instead and this parameter is ignored.
     ...
     * @param initializeShaderPrograms wether to initialize all the shader
     * programs.
     * You should supply a value of true unless you know what you're doing.
     ...
     * @throws NoContextAvailableException when no OpenGL context could be
     * attained.
     * @throws FunctionLoadException when the process of loading the OpenGL
     * funcion pointers failed.
     * @throws OpenGL::Framebuffer::Exception when the GeometryBuffer failed to
     * initialize.
     * @throws Font::InitException when the debug font is not present.
     */
    static void initialize(const char* windowTitle, Entity& cameraEntity,
     ...
     */
    inline static duration_type deltaTime() noexcept { return sDeltaTime; }

    /**
     * @brief Get the elapsed time since the Renderer initialized.
     * @return The elapsed time since the Renderer initialized.
     */
    inline static duration_type elapsedTime() noexcept { return sElapsedTime; }

    /**
     * @brief Get the elapsed and delta time in seconds.
     *
     * @param [out] elapsedTime This variable is set to the current elapsed
     * time.
    ...
    void submitMeshRenderer(const MeshRenderer* renderer);
    void submitAmbientLight(const AmbientLight* light);
     ...
     */
    inline static vec3f getCameraPosition() noexcept { return sCameraPosition; }

    /**
     * @brief Get the viewport width.
     * @return The viewport width.
     */
    inline static int width() noexcept { return sWidth; }

    /**
     * @brief Get the viewport height.
     * @return The viewport height.
     */
    inline static int height() noexcept { return sHeight; }
     ...
     */
    inline static float aspectRatio() noexcept { return sAspectRatio; }
     ...
     */
    static void setCursorPosition(const double x, const double y) noexcept;
     ...
     */
    static bool keyToggleRelease(const int keycode) noexcept;
    ...
    /**
     * @brief Set the root of the GUI elements to draw. You can pass a null
     * pointer
     * so that no GUI is drawn. The Renderer does not take ownership of the
     * GUI::Base,
    ...
    static void setEntityDebugShadowBuffer(
        std::shared_ptr<Entity> entityWithShadowBuffer) noexcept
    {
  ...
  private:
    ...
    static Entity* sCameraEntity;
    static Entity* sDebugShadowBufferEntity;
