...
{
    GINTONIC_ASSET_BOILERPLATE(ShaderProgram, "shaderprograms", ".shader");
  ...
  public:
    ...
    fromShaderFiles(std::string name, std::string vertexShaderFile,
                    std::string geometryShaderFile,
                    std::string fragmentShaderFile);

    void setVertexShaderFile(std::string vertexShaderFile);
    void setGeometryShaderFile(std::string geometryShaderFile);
    void setFragmentShaderFile(std::string fragmentShaderFile);

    const std::string& getVertexShaderFile() const noexcept;
    const std::string& getGeometryShaderFile() const noexcept;
    const std::string& getFragmentShaderFile() const noexcept;

    bool isDirty() const noexcept;

    bool compileAndLink(std::basic_string<GLchar>& errorMessage);
  ...
  private:
    bool attemptLink(std::basic_string<GLchar>& errorMessage);
    void initUniforms();

    bool mIsDirty = false;
    GLuint mHandle = 0;
    std::string mVertexShader, mGeometryShader, mFragmentShader;
  ...
  private:
    ...
    void save(Archive& archive, const unsigned /*version*/) const
    {
        ...
        archive << BOOST_SERIALIZATION_BASE_OBJECT_NVP(Asset)
                << make_nvp("vertex_shader", mVertexShader)
                << make_nvp("geometry_shader", mGeometryShader)
                << make_nvp("fragment_shader", mFragmentShader);
    }
    ...
    void load(Archive& archive, const unsigned /*version*/)
    {
        ...
        if (!compileAndLink(errorMessage))
        {
