...
template <GLuint Size> class BufferObjectArray
{
  public:
    /// Default constructor.
    inline BufferObjectArray() { glGenBuffers(Size, mHandles); }

    /// Destructor.
    inline ~BufferObjectArray() noexcept { glDeleteBuffers(Size, mHandles); }
    ...
    std::vector<Type, Allocator> retrieveDataAs(const std::size_t index,
                                                GLint& usage) const
    {
        GLint lSize;
...
template <GLuint Size>
BufferObjectArray<Size>& BufferObjectArray<Size>::
operator=(BufferObjectArray<Size>&& other)
{
