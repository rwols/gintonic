...
#include <gintonic/Graphics/OpenGL/Vector.hpp>
...
{
    GINTONIC_ASSET_BOILERPLATE(Mesh, "meshes", ".mesh");
  ...
  public:
    ...
    fromFbxMesh(FBXSDK_NAMESPACE::FbxMesh* fbxMesh);
     ...
     */
    static std::shared_ptr<Mesh> fromFbxMesh(FBXSDK_NAMESPACE::FbxMesh* fbxMesh,
                                             Skeleton& skeleton);
     ...
     * the same size.
     * The values of the index array must refer to valid indices in the position
     * and normal arrays.
     ...
     * are the position information for a vertex, and the w coordinate is the x
     * coordinate of the
     * texture coordinate information.
     * @param [in] normal_XYZ_uv_Y An array of four-dimensional vectors where
     * the x,y,z coordinates
     * are the normal vector information for a vertex, and the w coordinate is
     * the y coordinate of the
     * texture coordinate information.
     */
    static std::shared_ptr<Mesh>
    fromArrays(const std::vector<GLuint>& indices,
     ...
     * `tangent_XYZ_handedness` must
     * be of the same size.
     * The values of the index array must refer to valid indices in the position
     * and normal arrays.
     * The index array can be of any size. The w-coordinate of the
     * `tangent_XYZ_handedness` array
     * is the *handedness*. This works as follows: When you have the normal
     * vector, say `N`, and the
     * tangent vector, say `T`, then to get the bitangent `B` vector you can
     * simply calculate the cross
     * product of `N` and `T`. But some modeling software systems use a reversed
     * bitangent vector `-B`,
     * so you must record the `+1` or `-1` information in this last
     * w-coordinate. In a vertex shader,
     * the bitangent vector is retrieved with a cross product of `N` and `T` and
     * multiplied by the
     * handedness.
     * @param [in] indices The index array.
     * @param [in] position_XYZ_uv_X An array of four-dimensional vectors where
     ...
     * where the x,y,z coordinates
     * are the tangent vector information for a vertex, and the w coordinate is
  ...
  public:
     ...
     * @param [in] fbxMesh A constant pointer to an FBX mesh.
     */
    void set(FBXSDK_NAMESPACE::FbxMesh* fbxMesh);
     ...
     * @param [in] skeleton The skeleton to set up the bind poses for.
     */
    void set(FBXSDK_NAMESPACE::FbxMesh* fbxMesh, Skeleton& skeleton);
  ...
  private:
    ...
    void buildFromFbxMesh(FBXSDK_NAMESPACE::FbxMesh*,
                          std::multimap<int, GLuint>*);
    void buildSkinningInformation(const FBXSDK_NAMESPACE::FbxMesh*, Skeleton&,
                                  const std::multimap<int, GLuint>&);
