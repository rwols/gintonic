 ...
 * 16-byte boundary.
 */
union GINTONIC_EXPORT alignas(__m128) vec4f
{
  private:
  public:
    /// The raw SSE type.
    ...
    /// Default constructor.
    inline vec4f() noexcept { GT_PROFILE_FUNCTION; }

    /// Constructor that sets every coordinate to the given value.
    inline vec4f(const float s) noexcept : data(_mm_set1_ps(s))
    {
        GT_PROFILE_FUNCTION;
    }

    /// Constructor.
    inline vec4f(const float a, const float b, const float c,
    ...
    inline vec4f(__m128 values) noexcept : data(values) { GT_PROFILE_FUNCTION; }
    ...
    /// Move constructor.
    inline vec4f(vec4f&& v) noexcept : data(v.data) { GT_PROFILE_FUNCTION; }

    /// Constructor.
    vec4f(const vec2f& v, const float z, const float w) noexcept;
...
inline vec4f operator*(const float lhs, const vec4f& rhs) noexcept
{
...
inline float distance2(const vec4f& u, const vec4f& v) noexcept
{
    return (u - v).length2();
}

/// Get the distance between two vectors.
inline float distance(const vec4f& u, const vec4f& v) noexcept
{
    return (u - v).length();
}

/// Mix two vectors.
inline vec4f mix(const vec4f& u, const vec4f& v, const float a) noexcept
{
    return (1.0f - a) * u + a * v;
}
...
inline std::ostream& operator<<(std::ostream& os, const vec4f& v) noexcept
{
    return os << v.x << ' ' << v.y << ' ' << v.z << ' ' << v.w;
}

/// Input stream support for vectors.
inline std::istream& operator>>(std::istream& is, vec4f& v) noexcept
{
    is >> v.x >> v.y >> v.z >> v.w;
    return is;
}
