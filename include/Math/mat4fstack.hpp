 ...
 */
class GINTONIC_EXPORT mat4fstack
{
  public:
    /**
     * @brief Default constructor pushes the identity matrix on the stack.
     */
    inline mat4fstack() { m_stack.push(mat4f(1.0f)); }

    /**
     * @brief Constructor that pushes the given matrix onto the stack.
     * @param m Some matrix to start out with.
     */
    inline mat4fstack(const mat4f& m) { m_stack.push(m); }

    /**
     * @brief Constructor that pushes the given matrix onto the stack.
     * @param m Some matrix to start out with.
     */
    inline mat4fstack(mat4f&& m) { m_stack.push(std::move(m)); }
    ...
    /// Move constructor.
    inline mat4fstack(mat4fstack&& other) : m_stack(std::move(other.m_stack))
    {
        /* Empty on purpose. */
    }
     ...
     */
    inline mat4f& top() { return m_stack.top(); }

    /**
     * @brief Access the top element of the stack.
     * @return A const reference to the top element of the stack.
     */
    inline const mat4f& top() const { return m_stack.top(); }
     ...
     */
    inline void push(const mat4f& m) { m_stack.push(m_stack.top() * m); }
     ...
     * @param m The matrix to multiply the top matrix with.
     */
    inline void push(mat4f&& m) { m_stack.push(m_stack.top() * std::move(m)); }
     ...
     */
    inline void swap(mat4fstack& other) { m_stack.swap(other.m_stack); }
