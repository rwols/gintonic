...
struct GINTONIC_EXPORT box3f
{
    ...
    template <class OutputIter> void getCorners(OutputIter iter) const noexcept
    {
#if 1
        ...
        *iter = minCorner;
        ++iter;
        *iter = _mm_add_ps(minCorner.data, _mm_x000_ps(lDims));
        ++iter;
        *iter = _mm_add_ps(minCorner.data, _mm_0y00_ps(lDims));
        ++iter;
        *iter = _mm_add_ps(minCorner.data, _mm_00z0_ps(lDims));
        ++iter;
        *iter = _mm_add_ps(minCorner.data, _mm_0yz0_ps(lDims));
        ++iter;
        *iter = _mm_add_ps(minCorner.data, _mm_x0z0_ps(lDims));
        ++iter;
        *iter = _mm_add_ps(minCorner.data, _mm_xy00_ps(lDims));
        ++iter;
        *iter = maxCorner;
        ++iter;
        ...
        *iter = minCorner;
        ++iter;
        *iter = minCorner + vec3f(lDimensions.x, 0.0f, 0.0f);
        ++iter;
        *iter = minCorner + vec3f(0.0f, lDimensions.y, 0.0f);
        ++iter;
        *iter = minCorner + vec3f(0.0f, 0.0f, lDimensions.z);
        ++iter;
        *iter = minCorner + vec3f(0.0f, lDimensions.y, lDimensions.z);
        ++iter;
        *iter = minCorner + vec3f(lDimensions.x, 0.0f, lDimensions.z);
        ++iter;
        *iter = minCorner + vec3f(lDimensions.x, lDimensions.y, 0.0f);
        ++iter;
        *iter = maxCorner;
        ++iter;
