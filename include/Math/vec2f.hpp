 ...
 * 16-byte boundary.
 */
union GINTONIC_EXPORT alignas(__m128) vec2f
{
  private:
    __m128 data;
    friend union vec3f;
  ...
  public:
    ...
    /// Default constructor.
    inline vec2f() noexcept : dummy0(0.0f), dummy1(0.0f)
    {
        GT_PROFILE_FUNCTION;
    }

    /// Constructor that sets every coordinate to the given value.
    inline vec2f(const float s) noexcept : data(_mm_set1_ps(s))
    {
        GT_PROFILE_FUNCTION;
    }
    ...
    inline vec2f(__m128 values) noexcept : data(values) { GT_PROFILE_FUNCTION; }
    ...
    /// Move constructor.
    inline vec2f(vec2f&& v) noexcept : data(v.data) { GT_PROFILE_FUNCTION; }
    ...
    inline friend float dot(const vec2f& lhs, const vec2f& rhs) noexcept
    {
        GT_PROFILE_FUNCTION;
