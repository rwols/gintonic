 ...
 */
union GINTONIC_EXPORT alignas(__m128) mat4f
{
  public:
    /// The raw SSE array.
    __m128 data[4];
    ...
    mat4f(const float diagonal)
        : m00(diagonal), m10(0.0f), m20(0.0f), m30(0.0f), m01(0.0f),
          m11(diagonal), m21(0.0f), m31(0.0f), m02(0.0f), m12(0.0f),
          m22(diagonal), m32(0.0f), m03(0.0f), m13(0.0f), m23(0.0f),
          m33(diagonal)
    {
        GT_PROFILE_FUNCTION;
    }
    ...
    mat4f(const float scalex, const float scaley, const float scalez)
        : m00(scalex), m10(0.0f), m20(0.0f), m30(0.0f), m01(0.0f), m11(scaley),
          m21(0.0f), m31(0.0f), m02(0.0f), m12(0.0f), m22(scalez), m32(0.0f),
    ...
    /// Column constructor.
    mat4f(const __m128& column0, const __m128& column1, const __m128& column2,
          const __m128& column3)
    {
        GT_PROFILE_FUNCTION;
    ...
    /// Column constructor.
    mat4f(const vec4f& column0, const vec4f& column1, const vec4f& column2,
          const vec4f& column3);
    ...
    /// Look-at constructor.
    mat4f(const vec3f& eye_location, const vec3f& subject_location,
          const vec3f& up_direction);

    /// Set this matrix as an orthographic projection matrix.
    mat4f& set_orthographic(const float left, const float right,
                            const float bottom, const float top,
                            const float nearplane, const float farplane);

    /// Set this matrix as an orthographic projection matrix.
    mat4f& set_orthographic(const float width, const float height,
                            const float nearplane, const float farplane);

    /// Set this matrix as a perspective projection matrix.
    mat4f& set_perspective(const float fieldofview, const float aspectratio,
                           const float nearplane, const float farplane);

    /// Set this matrix as an infinite perspective projection matrix.
    mat4f& set_perspective_infinite(const float fieldofview,
                                    const float aspectratio,
                                    const float nearplane);

    /// Set this matrix as the inverse of a perspective projection matrix (so
    /// essentially a frustum).
    mat4f& set_inverse_perspective(const float fieldofview,
                                   const float aspectratio,
                                   const float nearplane, const float farplane);

    /// Assuming this matrix is a perspective projection, retrieve the
    /// parameters.
    void unproject_perspective(float& fieldofview, float& aspectratio,
                               float& nearplane, float& farplane);

    /// Assuming this matrix is an affine transformation, decompose it into its
    /// scale, rotation and translation components.
    ...
    inline mat4f operator+(const mat4f& other) const noexcept
    {
        ...
        return mat4f(_mm_add_ps(data[0], other.data[0]),
    ...
    inline mat4f operator-(const mat4f& other) const noexcept
    {
        ...
        return mat4f(_mm_sub_ps(data[0], other.data[0]),
    ...
    inline mat4f operator*(const float s) const noexcept
    {
        ...
        return mat4f(_mm_mul_ps(data[0], tmp), _mm_mul_ps(data[1], tmp),
                     _mm_mul_ps(data[2], tmp), _mm_mul_ps(data[3], tmp));
    }
    ...
    inline friend mat4f operator*(const float s, const mat4f& m) noexcept
    {
        ...
        const auto tmp = _mm_set1_ps(s);
        return mat4f(_mm_mul_ps(tmp, m.data[0]), _mm_mul_ps(tmp, m.data[1]),
                     _mm_mul_ps(tmp, m.data[2]), _mm_mul_ps(tmp, m.data[3]));
    }
...
inline std::istream& operator>>(std::istream& is, mat4f& m)
{
    ...
    is >> m.m00 >> m.m10 >> m.m20 >> m.m30 >> m.m01 >> m.m11 >> m.m21 >>
        m.m31 >> m.m02 >> m.m12 >> m.m22 >> m.m32 >> m.m03 >> m.m13 >> m.m23 >>
        m.m33;
    return is;
}
