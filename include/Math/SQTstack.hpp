 ...
 */
class GINTONIC_EXPORT SQTstack
{
  public:
    /**
     ...
     */
    inline SQTstack(const SQT& sqt) { m_stack.push(sqt); }
     ...
     * @param sqt The SQT to initialize the stack with.
     */
    inline SQTstack(SQT&& sqt) { m_stack.push(std::move(sqt)); }
     ...
     */
    inline SQTstack(SQTstack&& other) : m_stack(std::move(other.m_stack))
    {
        /* Empty on purpose. */
    }
    ...
    /**
     * @brief Initialize an SQTstack with a scaling, a rotation and a
     * translation.
     *
     ...
     * @param u2 The translation. This gets forwarded to the constructor of
     * vec3f.
     */
    template <class U0, class U1, class U2> SQTstack(U0&& u0, U1&& u1, U2&& u2)
    {
        m_stack.emplace(std::forward<U0>(u0), std::forward<U1>(u1),
                        std::forward<U2>(u2));
    }
     ...
     */
    inline SQT& top() { return m_stack.top(); }

    /**
     * @brief Access the top element of the stack.
     * @return A constant reference to the top element of the stack.
     */
    inline const SQT& top() const { return m_stack.top(); }

    /**
     * @brief Push a default-constructed SQT onto the stack.
     */
    inline void push() { m_stack.push(m_stack.top()); }

    /**
     * @brief Add the SQT with the top of the stack and push the result onto the
     * stack.
     *
     * @param sqt The SQT.
     */
    inline void push(const SQT& sqt) { m_stack.push(m_stack.top() % sqt); }
    ...
    /**
     * @brief Add the SQT constructed with the top of the stack and push the
     ...
     */
    inline void push(const vec3f& scale, const quatf& rotation,
     ...
     */
    inline void swap(SQTstack& other) { m_stack.swap(other.m_stack); }
