 ...
 * 16-byte boundary.
 */
union GINTONIC_EXPORT alignas(__m128) vec3f
{
  public:
    /// The raw SSE type.
    __m128 data;
    ...
    /// Default constructor.
    inline vec3f() noexcept : dummy(0.0f)
    {
        GT_PROFILE_FUNCTION;
    ...
    inline vec3f(const float s) noexcept : data(_mm_set1_ps(s))
    {
        GT_PROFILE_FUNCTION;
    }

    /// Constructor.
    inline vec3f(const float a, const float b, const float c) noexcept
    ...
    inline vec3f(__m128 values) noexcept : data(values) { GT_PROFILE_FUNCTION; }
    ...
    /// Move constructor.
    inline vec3f(vec3f&& v) noexcept : data(v.data) { GT_PROFILE_FUNCTION; }
    ...
    inline friend vec3f cross(const vec3f& lhs, const vec3f& rhs)
    {
        ...
        auto result = _mm_sub_ps(
            _mm_mul_ps(lhs.data, _mm_shuffle_ps(rhs.data, rhs.data,
                                                _MM_SHUFFLE(3, 0, 2, 1))),
            _mm_mul_ps(rhs.data, _mm_shuffle_ps(lhs.data, lhs.data,
                                                _MM_SHUFFLE(3, 0, 2, 1))));
...
inline vec3f operator*(const float lhs, const vec3f& rhs) noexcept
{
...
inline float distance2(const vec3f& u, const vec3f& v) noexcept
{
    return (u - v).length2();
}

/// Get the distance between two vectors.
inline float distance(const vec3f& u, const vec3f& v) noexcept
{
    return (u - v).length();
}

/// Mix two vectors.
inline vec3f mix(const vec3f& u, const vec3f& v, const float a) noexcept
{
    return (1.0f - a) * u + a * v;
}
...
inline std::ostream& operator<<(std::ostream& os, const vec3f& v) noexcept
{
    return os << v.x << ' ' << v.y << ' ' << v.z;
}

/// Input stream support for vectors.
inline std::istream& operator>>(std::istream& is, vec3f& v) noexcept
{
    is >> v.x >> v.y >> v.z;
    return is;
}
