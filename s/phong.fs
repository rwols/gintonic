...
#define SPECULAR_SHININESS 4.0f
...
uniform sampler2D texture_diffuse; // the w-component is the specularity factor
...
void main()
{
    vec4 the_diffuse_texture = texture(texture_normal, model_uv);
    ...
    if (the_light_type == LIGHT_TYPE_DIRECTIONAL)
    {
        ...
        float spec_angle = max(dot(reflect_dir, view_dir), 0.0f);
        float specular = lambertian * the_diffuse_texture.w * pow(spec_angle, SPECULAR_SHININESS);
        final_color = (lambertian + specular) * light_intensity * the_diffuse_texture.rgb;
    }
