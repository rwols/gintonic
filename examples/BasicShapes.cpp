...
class BasicShapesApplication : public gintonic::Application
{
  public:
    std::shared_ptr<gintonic::Entity> mRootOfSolidShapes;

    BasicShapesApplication(int argc, char** argv) : Application(argc, argv)
    {
  ...
  private:
    virtual void onRenderUpdate() final
    {
        ...
        {
            lShape->setRotation(quatf::axis_angle(
                vec3f(0.0f, 1.0f, 0.0f), static_cast<float>(mElapsedTime)));
        }

        // All the simple solids are a child of the "invisible" root entity,
        // centered at the origin. So we apply a rotation to that too, in order
        // to make the solids "spin around".
        mRootOfSolidShapes->setRotation(quatf::axis_angle(
            vec3f(0.0f, 1.0f, 0.0f), -static_cast<float>(mElapsedTime) / 4.0f));
    }
};
