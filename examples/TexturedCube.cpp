...
class TexturedCubeApplication : public gintonic::Application
{
  public:
  ...
  private:
    ...
    virtual void onRenderUpdate() final
    {
        ...
        const auto lYAxis =
            (1.0f + std::cos(static_cast<float>(mElapsedTime))) / 2.0f;
        const auto lZAxis =
            (1.0f + std::sin(static_cast<float>(mElapsedTime))) / 2.0f;
        const auto lRotationAxis = vec3f(0.0f, lYAxis, lZAxis).normalize();
        const auto lQuaternion = quatf::axis_angle(
            lRotationAxis, -static_cast<float>(mElapsedTime) / 4.0f);
...
DEFINE_MAIN(TexturedCubeApplication, "TexturedCube",
            "Draws a rotating textured cube.")
