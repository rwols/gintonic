...
{
  public:
    EntityProcessor(std::shared_ptr<gintonic::Entity> root,
                    std::ostream& stream, const std::size_t tabwidth,
                    std::vector<std::shared_ptr<gintonic::Entity>>& lights)
        : EntityVisitor(root), mStream(stream), mTabWidth(tabwidth),
          mLights(lights)
    {
    }
  ...
  private:
    ...
    virtual bool onVisit(std::shared_ptr<gintonic::Entity> entity)
    {
        ...
        if (entity->mesh)
        {
            ...
            auto lBBoxArea = (lBBox.maxCorner.x - lBBox.minCorner.x) *
    ...
    virtual void onFinish() { niceMoveSpeed /= static_cast<float>(numMeshes); }
...
class FbxViewerApplication : public gintonic::Application
{
  public:
        ...
        : gintonic::Application(argc, argv)
    {
        ...
        if (argc <= 2)
        {
            throw exception("Supply an FBX filename! Note: The first argument "
                            "is wether we go fullscreen, the second argument "
                            "is the FBX filename. This example also checks if "
                            "there's already something in the cache with the "
                            "same filename.");
        }
        ...
        const auto lCachedFile =
            "Resources" /
            boost::filesystem::path(lFilename.stem().string() + ".entity");
        ...
        else if (boost::filesystem::is_regular_file(lCachedFile))
        {
            ...
            const std::time_t lFilenameDate =
                boost::filesystem::last_write_time(lFilename);
            const std::time_t lCachedFileDate =
                boost::filesystem::last_write_time(lCachedFile);
            if (lCachedFileDate < lFilenameDate)
            {
                std::cout
                    << "The cache is out of date. Loading original file.\n";
        ...
        LABEL_CacheIsOutOfDate:
            ...
            const auto lFilenameAsString = lFilename.string();
            gintonic::FbxImporter lImporter(lFilenameAsString.c_str(), true,
                                            false, true);
            mModel = lImporter.loadEntities();

            // Serialize the model for caching
            std::ofstream lOutput("assets/" + mModel->name + ".entity",
                                  std::ios::binary);
        ...
        EntityProcessor lEntityProcessor(mRootEntity, std::cout, 4, mLights);

        std::cout << "\n\tHere's an overview of the scene:\n\n";
        lEntityProcessor.execute();
        if (lEntityProcessor.containsLight == false)
        {
            std::cout << "\nScene contains no lights! Adding a directional "
                         "light...\n";
        ...
        Renderer::getCameraEntity()->setTranslationZ(
            lEntityProcessor.cameraOffsetZ + 4.0f);
  ...
  private:
    ...
    virtual void onRenderUpdate() final
    {
        ...
        if (mRotateModel)
        {
            mModel->postMultiplyRotation(
                quatf::axis_angle(vec3f(0.0f, 1.0f, 0.0f),
                                  static_cast<float>(mDeltaTime) / 10.0f));
        }
    }
};
