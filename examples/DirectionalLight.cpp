...
class DirectionalLightApplication : public gintonic::Application
{
  public:
    std::shared_ptr<gintonic::Entity> mCubeEntity;

    DirectionalLightApplication(int argc, char** argv) : Application(argc, argv)
    {
        ...
        mCubeEntity->material->diffuseTexture =
            Texture2D::fromImage(imageOptions);
        imageOptions.relativeFilename = "assets/images/RuralBrickWall_SPEC.png";
        mCubeEntity->material->specularTexture =
            Texture2D::fromImage(imageOptions);
        imageOptions.relativeFilename = "assets/images/RuralBrickWall_NRM.png";
        mCubeEntity->material->normalTexture =
            Texture2D::fromImage(imageOptions);
  ...
  private:
    ...
    {
        using namespace gintonic;
        mCubeEntity->postMultiplyRotation(quatf::axis_angle(
            vec3f(1.0f, 0.0f, 0.0f), static_cast<float>(mDeltaTime) / 10.0f));
        mCubeEntity->postMultiplyRotation(quatf::axis_angle(
            vec3f(0.0f, 1.0f, 0.0f), static_cast<float>(mDeltaTime) / 5.0f));
    }
};

DEFINE_MAIN(DirectionalLightApplication, "DirectionalLight",
