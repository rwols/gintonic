...
box3f Octree::getBounds(const Entity& entity) const noexcept
{
    ...
    else
    {
        ...
        {
            return box3f(transform->getGlobalPosition(),
                         transform->getGlobalPosition());
        }
        else
        {
            return box3f(vec3f(0, 0, 0), vec3f(0, 0, 0));
        }
    }
}

// Constructors.
Octree::Octree(const box3f& bounds) : mBounds(bounds)
{
...
Octree::Octree(const Octree& other)
    : mBounds(other.mBounds), mEntities(other.mEntities)
{
    if (other.mAllocationPlace)
    {
        ...
        {
            mChild[0] =
                new ((Octree*)mAllocationPlace + c) Octree(*(other.mChild[c]));
        }
    }
    else
    {
...
Octree::Octree(Octree&& other)
    : mAllocationPlace(other.mAllocationPlace), mParent(other.mParent),
      mBounds(std::move(other.mBounds)), mEntities(std::move(other.mEntities))
{
    ...
    other.mChild[0] = other.mChild[1] = other.mChild[2] = other.mChild[3] =
        other.mChild[4] = other.mChild[5] = other.mChild[6] = other.mChild[7] =
            nullptr;
}
...
bool Octree::isLeaf() const noexcept { return mAllocationPlace == nullptr; }
...
{
    if (!mBounds.contains(getBounds(entity)))
    {
        throw EntityNotContainedInOctreeBoundingBox(this, entity);
    }
    ...
    if (!isLeaf())
    {
        ...
        {
            if (childNode->mBounds.contains(getBounds(entity)))
            {
    ...
    auto holderIter = std::prev(mEntities.end());
    ...
    auto lTransformChangeConnection = entity->onTransformChange.connect(
        [this, holderIter](Entity::SharedPtr thisEntity) {
            if (auto lSharedPtr = holderIter->entity.lock())
            {
                auto lUpmostParent = this->erase(holderIter);
    ...
    holderIter->transformChangeConnection =
        std::move(lTransformChangeConnection);

    // Subscribe to the onDie event of the Entity.
    // When the Entity dies (i.e. destructor is called), we need to
    // be notified of that event so that we remove the Entity from the octree
    // node too.
    auto lDieConnection =
        entity->onDie.connect([this, holderIter](Entity* thisEntity) {
            this->erase(holderIter);
            // assert(this == thisEntity->mOctree);
            // #ifdef NDEBUG
    ...
    holderIter->destructConnection = std::move(lDieConnection);

    // mEntities.emplace_back(entity, std::move(lTransformChangeConnection),
    // std::move(lDieConnection));
...
Octree* Octree::backRecursiveDelete()
{
    ...
    {
        lEraseChildren += childNode->hasNoEntities() ? 1 : 0;
        lEraseChildren += childNode->isLeaf() ? 1 : 0;
    }
    // DEBUG_PRINT;
    if (lEraseChildren == 16)
    {
        ...
        for (auto*& childNode : mChild)
        {
            (Octree::operator delete)(childNode, mAllocationPlace);
            childNode->~Octree();
            childNode = nullptr;
        }
        // DEBUG_PRINT;
...
Octree::Octree(const float subdivisionThreshold, Octree* parent,
               const box3f& bounds)
    : mParent(parent), mBounds(bounds),
...
Octree::Octree(const float subdivisionThreshold, Octree* parent,
    ...
    : mParent(parent), mBounds(min, max),
      subdivisionThreshold(subdivisionThreshold)
{
    mChild[0] = mChild[1] = mChild[2] = mChild[3] = mChild[4] = mChild[5] =
        mChild[6] = mChild[7] = nullptr;
...
void Octree::subdivide()
{
    ...
    mChild[0] = new ((Octree*)mAllocationPlace + 0)
        Octree(subdivisionThreshold, this, lMin, lMin + lHalf);
    lMin.x += lHalf.x;
    mChild[1] = new ((Octree*)mAllocationPlace + 1)
        Octree(subdivisionThreshold, this, lMin, lMin + lHalf);
    lMin.y += lHalf.y;
    mChild[2] = new ((Octree*)mAllocationPlace + 2)
        Octree(subdivisionThreshold, this, lMin, lMin + lHalf);
    lMin.x -= lHalf.x;
    mChild[3] = new ((Octree*)mAllocationPlace + 3)
        Octree(subdivisionThreshold, this, lMin, lMin + lHalf);
    lMin.y -= lHalf.y;
    // at this point, we are at the original location of lMin
    lMin.z += lHalf.z;
    mChild[4] = new ((Octree*)mAllocationPlace + 4)
        Octree(subdivisionThreshold, this, lMin, lMin + lHalf);
    lMin.x += lHalf.x;
    mChild[5] = new ((Octree*)mAllocationPlace + 5)
        Octree(subdivisionThreshold, this, lMin, lMin + lHalf);
    lMin.y += lHalf.y;
    mChild[6] = new ((Octree*)mAllocationPlace + 6)
        Octree(subdivisionThreshold, this, lMin, lMin + lHalf);
    lMin.x -= lHalf.x;
    mChild[7] = new ((Octree*)mAllocationPlace + 7)
        Octree(subdivisionThreshold, this, lMin, lMin + lHalf);
}
