...
Skeleton::Joint::Joint(const FbxNode* pFbxNode, const uint8_t parentIndex)
    : name(pFbxNode->GetName()), inverseBindPose(1.0f), parent(parentIndex)
{
    for (std::size_t c = 0; c < GT_JOINT_MAX_CHILDREN; ++c)
    {
...
Skeleton::Skeleton(const FbxScene* pFbxScene)
    : Skeleton(pFbxScene->GetRootNode())
{
}

Skeleton::Skeleton(const FbxNode* pFbxNode) : name(pFbxNode->GetName())
{
...
void Skeleton::constructJointsRecursive(const FbxNode* pNode,
