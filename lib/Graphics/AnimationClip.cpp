...
template <class FbxType> class FbxInConstCollection
{
  public:
    ...
    FbxInConstCollection(const FbxCollection* collection)
        : mCollection(collection)
    {
        if (collection == nullptr)
            throw std::invalid_argument("Collection is a nullptr");
    }

    class const_iterator
        : public std::iterator<std::random_access_iterator_tag, const FbxType*,
                               int, const FbxType*, void>
    {
      public:
        using Super = std::iterator<std::random_access_iterator_tag,
                                    const FbxType*, int, const FbxType*, void>;
        ...
        inline const FbxType* operator*() const noexcept
        {
            return mCollection->GetMember<FbxType>(mCurrent);
        }
        inline const FbxType* operator->() const noexcept
        {
            return mCollection->GetMember<FbxType>(mCurrent);
        ...
        {
            return const_iterator(mCollection, mCurrent + 1);
        }
        inline const_iterator& operator--() noexcept
        {
            --mCurrent;
        ...
        inline const_iterator& operator+=(const difference_type d) noexcept
        {
            mCurrent += d;
            return *this;
        }
        inline const_iterator operator+(const difference_type d) const noexcept
        {
            return const_iterator(mCollection, mCurrent + d);
        }
        inline const_iterator& operator-=(const difference_type d) noexcept
        {
            mCurrent -= d;
        ...
        inline value_type operator[](const const_iterator& other) const noexcept
        {
            return *(*this + other.mCurrent);
        }

        inline bool operator==(const const_iterator& other) const noexcept
        {
            return mCollection == other.mCollection &&
      ...
      private:
        ...
        const_iterator(const FbxCollection* collection,
                       const difference_type current)
            : mCollection(collection), mCurrent(current)
        {
        }
    ...
    {
        return const_iterator(mCollection, 0);
    }
    const_iterator end() const noexcept
    {
        return const_iterator(mCollection,
                              mCollection->GetMemberCount<FbxType>());
    }
...
template <class FbxType> class FbxInCollection
{
  public:
    ...
    class iterator : public std::iterator<std::random_access_iterator_tag,
                                          FbxType*, int, FbxType*, void>
    {
      public:
        using Super = std::iterator<std::random_access_iterator_tag, FbxType*,
                                    int, FbxType*, void>;
        using iterator_category = typename Super::iterator_category;
        using value_type = typename Super::value_type;
        using difference_type = typename Super::difference_type;
        ...
        {
            return iterator(mCollection, mCurrent + 1);
        }
        inline iterator& operator--() noexcept
        {
            --mCurrent;
        ...
        inline iterator& operator+=(const difference_type d) noexcept
        {
            mCurrent += d;
            return *this;
        }
        inline iterator operator+(const difference_type d) const noexcept
        {
            return iterator(mCollection, mCurrent + d);
        }
        inline iterator& operator-=(const difference_type d) noexcept
        {
            mCurrent -= d;
        ...
        inline value_type operator[](const iterator& other) const noexcept
        {
            return *(*this + other.mCurrent);
        }
      ...
      private:
        ...
        iterator(FbxCollection* collection, const difference_type current)
            : mCollection(collection), mCurrent(current)
        {
        }
    ...
    {
        return iterator(mCollection, mCollection->GetMemberCount<FbxType>());
    }
...
AnimationClip::AnimationClip(FbxAnimStack* pStack,
                             std::shared_ptr<Skeleton> targetSkeleton)
    : name(pStack->GetName()), skeleton(targetSkeleton)
{
    ...
    for (uint8_t j = 0; j < targetSkeleton->joints.size(); ++j)
    {
        ...
        {
            std::cerr << "\tWARNING: Could not find joint \""
                      << lSkeleton[j].name << "\".\n";
            continue;
        }
        ...
        {
            const auto& lAffineMatrix =
                lEvaluator->GetNodeLocalTransform(lFbxJoint, t);
