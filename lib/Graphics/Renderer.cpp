...
std::vector<MeshRenderer*> sMeshRenderers;
...
{
  public:
    EntitySorter(Entity& root) : EntityVisitor(root) { sEntitiesLock.obtain(); }

    ~EntitySorter() override { sEntitiesLock.release(); }

    bool onVisit(Entity& entity) override
    {
    ...
    std::vector<mat3f>& matrixBNs) noexcept
{
    ...
    for (const auto& lEntity : geometries)
    {
        ...
        // glGetUniformBlockIndex(lMaterialShaderProgram, "JointBlock44");
        // const auto lNormalJointBlockIndex =
